...
[
        ...
        body('court',"please enter valid court name").isLength({min:5})
],
async (req,res)=>{
    let success= false;
    const result = validationResult(req);
...
try{
    let {lawyerId,bio,court,exp,city,ratings,practise} = req.body;
 ...
 user = await Lawyer.create({
   lawyerId:req.body.lawyerId,bio,court,exp,city,ratings,practise
});
const data = {
            user:{
                id:req.params.id
...
//api for login of client/lawyer
router.post('/login',[
