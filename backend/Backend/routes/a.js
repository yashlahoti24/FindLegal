...
router.post(
  ...
  [
    ...
    body("password", "password length should be atleast 6 characters").isLength(
      { min: 5 }
    ),
    ...
    try {
      ...
      if (!user) {
        ...
        if (!user)
          return res
            .status(404)
            .json({ success, err: "please enter valid credentials email" });
      }
      ...
      if (!passwordCompare) {
        return res
          ...
          .json({
            success,
            msg: "please try to login with correct creditenials",
          });
      }
      const payLoad = {
        user: {
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(404)
          .json({ success, err: "please enter valid credentials lawyerId" });
      }
